...
are pointed out. In most cases, the notation used conforms to \cite{MBTE}, which also provides an
...
To conduct practical scattering calculations, the energy-domain representation
of the propagator
...
\subsection{Perturbation Expansion and The Dyson Equation}
...
$\hat{H_{1}}$ iteratively applied to the non-interacting ground state. The
...
\footnotetext{The standard notations $\pi$ and $\nu$ refer to proton and neutron subshells, respectively.}
This parameter splits these subshells' occupants into upper and lower sublevels with energies 
$E_{\pm}$:
\begin{equation} \label{LevelSplitting}
...
\section{Parameterization of the Potential} \label{PotentialParameterization}
...
\begin{equation} \label{WoodsSaxon}
    \begin{split}
        f_{vol}(r; \bm{r_{0}}, \bm{a}) & = \frac{-1}{1+e^{(r-\bm{R})/\bm{a}}},\\
        \\
        f_{sur}(r; \bm{r_{0}}, \bm{a}) & = \frac{1}{r}\frac{d}{dr}f_{vol}(r; \bm{r_{0}},
        \bm{a}).
    \end{split}
\end{equation}
...
is simpler as the same potential is used for protons and neutrons, excepting Coulomb. For asymmetric systems, we
...
\end{figure}
The Coulomb term is calculated using the same experimentally-derived charge
...
\begin{figure}[tb]
    ...
    \caption[Energy dependence of the imaginary and real-dispersive parts of the potential]
    {
        Typical energy dependence of the imaginary part and the
        dispersion-relation-generated energy-dependent real part of the
        neutron optical potential are shown for \niEight.
    }
...
for the neutron imaginary potential \niEight\ is shown in Fig. \ref{VolumeIntegral}.
...
\mega\electronvolt\ upward, keep flat beyond 200
