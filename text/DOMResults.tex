...
8.5 \mega\electronvolt\per{A}, the experimental binding energy for \caForty. In this
...
calculated skins.
\begin{table}[tb] \label{NeutronSkins}
...
($W_{sur}^{i}$); in fits with a high weight for the RMS charge radius,
$W_{sur}^{i}$ would tend toward zero or even cross into negative territory, an
...
Interestingly, the volume integral of the imaginary strength (an integration over the
radial form factors) shows the imaginary potential to be largely symmetric
about the Fermi energy within roughly 30 MeV (see Fig. \cite{CaVolumeIntegral}),
...
issues that affect both the neutron and proton matter distributions will not
