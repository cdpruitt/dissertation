...
software. By taking data from the left and right detectors separately (shown
figs. \ref{LRCorrelation} and \ref{LRTimeDifferenceLinear}), we could assigned an
inherent timing resolution of our after our software CFD: 0.36 ns FWHM.
...
\begin{figure*}
    ...
    {Fine times for left and right PMT events, taken separately, of the time-of-flight 
        detector as recovered by our software CFD algorithm. The times given are referenced
        to the start of each event waveform, which varied event-to-event. The spread from 35-
        55 ns depends on where the digitizer initiated the waveform for each
        event and does not indicate timing resolution.}
    \label{LRCorrelation}
\end{figure*}
...
\begin{figure*}
    ...
    \caption[Time difference between left and right PMTs of time-of-flight detector] {
        For all events in a diagnostic data run, the time difference   
...
\begin{figure*}
    ...
    \caption{Effect of energy gating on left/right PMT time resolution}
    \label{LRTimeDifferenceByEnergy}
\end{figure*}

The energy dependence of the left-right time difference is shown in Fig. \ref{LRDifferenceByEnergy}.
...
have been propagated through subsequent analysis and appear in the systematic
uncertainty.
