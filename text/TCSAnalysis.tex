...
Including the actual beam energy profile, the average left-right time difference was
0.52 ns.
...
calculated times-of-flight \ref{RFTimeStudy}.
...
of the previous event's processing period, and thus be ignored. In reality, the
...
\cite{Abfalterer2001}. Both the fraction of T$_{0}$ signals that result in looking
...
dead-time", are tabulated. Time-of-flight histograms are scaled by these
...
only one event can be detected per micropulse anyway. Of course, it means
that only one event can be recorded per micropulse, so many events are discarded.
In addition, it requires that the analytic dead-time is
very precisely known, as corrections can soar to over 100\% if the beam flux is
high.
...
digitizer is still busy from a trigger $j$ bins ago. Moore also provides a more advanced 
formula to generate the appropriate dead-time correction in cases where the variation in beam 
flux is significant \ref{Moore1980}. However, an examination of our flux-per-micropulse data 
...
could be over 100\% for the lowest-energy neutrons, depending on beam flux.
%The effect of this correction on the final 
...
Could mention more systematic differences between \cite{Shane2010, Finlay1993, Abfalterer2001}?
- sample diameters and collimation diameter (2.2-2.9 cm in finlay)
- flight path length: 38.14 m in Finlay et al.
...
suggesting that backshine of neutrons into the flux monitor from the samples, or other 
physical mechanisms, were not the cause.
...
Residuals between our data and literature
...
IsotopicDiffSn}, respectively. By examining relative differences between
...
(n,p) cross section and resonance structure of Zn, Be, and \sixO contribute to
...
\niEight \cite{Perey1993} and at only one data point, 14.2 MeV, for \niFour
