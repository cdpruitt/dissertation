...
The DT-5730 provides leading-edge discrimination (\gls{LED}) and constant-fraction discrimination
...
are shown in Figs. \ref{LRCorrelation} and \ref{LRTimeDifferenceByEnergy}. 
...
\begin{figure}[tb]
    ...
    {Fine times for left and right PMT events, taken separately, of the TOF 
        ...
        Thus the spread from 35-55 ns depends
        on where the digitizer initiated the waveform for each
...
The uncertainty of our macropulse start time manifests as a spread in average $\gamma$-ray
times-of-flight. Were the start time exactly known for each macropulse, and the
micropulse period exactly known, the TOF for each $\gamma$ ray would be
exactly the same (if detector time resolution is ignored). The difference between the 
average calculated $\gamma$-ray times-of-flight (using the imprecise macropulse start
...
\begin{figure}[tb]
    ...
    {The effects of timing corrections on the $\gamma$-ray
        ...
        achieved in our Ca study \cite{Shane2010}, which also employed
        $\gamma$-averaging.}
...
time. Given the extremely small areal density of our samples, this meant we required a rate
...
In reality, the
problem is far worse, as the instantaneous flux is much higher during the $\gamma$-ray
...
busy'' signal),
the looking period was aborted. During a looking period, if a single neutron or $\gamma$-ray
...
\begin{figure}[tb]
    ...
    \caption[``Looking period'' logic from previous neutron \tot\ measurements at LANSCE]
    {
        ...
        evenly-spaced 1.8-\micro\second\ time intervals
        define a time frame 1.4-1.6 \micro\second\ long. For each
...
\begin{figure}[tb]
    ...
    \caption[Time difference between consecutive events in the time-of-flight detector]
...
those of previous LANSCE measurements \cite{Finlay1993, Abfalterer2001, Shane2010}, namely the
...
of the sample, and $\rho$ is the number density of atoms in the sample.
...
was approximately 2 mb  at energies above 100 \mega\electronvolt. The cross section for $^{64}$Ni was corrected for the 
...
thresholds during data production, applied various software thresholds, gated events by low- 
...
region, even a sub-nanosecond timing difference between the spectrum for the
blank and for the sample of interest is amplified into a large error in the
...
a higher data rate as fewer neutrons are absorbed or scattered in-flight than
when samples are in the beam path. By reducing the rate of data acquisition
...
In contrast to the case of O, no isotopic Ni data were available at all above 100
\mega\electronvolt and high-precision literature data were only available up to 15 \mega\electronvolt\ for
...
Similar to Ni isotopes, the only existing Sn isotope neutron \tot\ data were
below 25 \mega\electronvolt\ for both \snTwelve\ and
\snFour\ \cite{Harper1982, Timokhov1989, Rapaport1980, Dukarevich1967}.
Our results extended the energy range to 400 \mega\electronvolt\ and are
