...
$t$, the isospin, and treating protons and neutrons as generic "nucleons" with differing $t$
projections.
...
nature of constituent nucleons not explicitly included, so the LDM is  
...
[wrap up by mentioning failure to constrain symmetry energy and failure to deal with reactions]
...
(spins, parities, magnetic moments) are recovered. For non-spherical nuclei (in open shells),
deformation effects must be included as well as they alter the ordering of single-particle levels
and change the ground-state configuration. In addition, nucleons can be
...
\oSix data from \cite{Leuschner1994}
