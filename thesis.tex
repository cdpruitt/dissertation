...
%\documentclass[letterpaper, 12pt,twoside,draft]{book}
...
\usepackage[top=1truein, bottom=1.25truein, left=1truein, right=1truein, includefoot, includehead]{geometry}
...
\usepackage[symbols,toc]{glossaries} % makeglossaries
%\usepackage[postdot]{glossaries-extra} % for listing the word “page(s)” in the glossary entries
...
\usepackage{booktabs}
\usepackage{dcolumn}
%\usepackage[lofdepth,lotdepth]{subfig} % for aligned subfigures in appendices
\usepackage{wrapfig}
\usepackage{multirow}
...
% prevent hyphenation of uppercase words (acronyms)
\uchyph=0
...
\newcolumntype{M}{>{\centering\arraybackslash}m{1.1in}}
\newcolumntype{G}{>{\centering\arraybackslash}m{0.8in}}
...
\interfootnotelinepenalty=10000 % To make sure footnotes don't get split on a separate page, 10000 is max
...
\chapter{Neutron Elastic Differential Cross Sections: Experimental Setup} \label{ECSExperiment}
\input{\textDirectory/ECSExperiment}

\chapter{Neutron Elastic Differential Cross Sections:\protect\\ Analysis and Results} \label{ECSAnalysis}
...
\begin{appendices}
    ...
    \chapter{Isotopic Composition of Enriched Samples} \label{SampleComposition}
    \input{\textDirectory/composition}

    \newcommand{\sectionbreak}{\clearpage}

    \chapter{Experimental Data Used to Constrain DOM Potentials} \label{DOMDataSets}
    \input{\textDirectory/dataSets}

    \fancyhead{} %clears the header
    \fancyhead[RO]{\MakeUppercase{Appendix \thechapter: \ \leftmark}} %Right odd header
    \fancyhead[LE]{\MakeUppercase{Appendix \thechapter: \ \leftmark}} %Left even header

    \chapter{Parameter Values of DOM Potentials} \label{DOMParameters}
    ...
    \chapter{Visualization of DOM Fit Results} \label{DOMVisualization}
